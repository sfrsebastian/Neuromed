...
public class PacienteApi extends Controller {
	...
	public static Result darTodos(){
		...
		List<Paciente> pacientes = JPA.em().createQuery("SELECT u FROM Paciente u WHERE u.doctor IS NULL ORDER BY u.nombre", Paciente.class).getResultList();
	...
	public static Result darEpisodiosPorFecha(Long idPaciente, String inic, String fi){
        ...
        if (SecurityController.validateOnlyMeWithDoctor(user)) {
            ...
            try {
                ...
                List<Episodio> episodios = JPA.em().createQuery("SELECT e FROM Episodio e WHERE e.paciente = ?1 AND e.fecha BETWEEN ?2 AND ?3 AND (e.doctor = ?4 OR ?4 MEMBER OF e.doctores) ORDER BY e.fecha", Episodio.class).setParameter(1, paciente).setParameter(2, inicio).setParameter(3, fin).setParameter(4,user).getResultList();
                return ok(Episodio.listToJson(episodios));
            }
            catch(Exception e){
                return status(StatusMessages.C_BAD_REQUEST, StatusMessages.M_INCORRECT_PARAMS);
